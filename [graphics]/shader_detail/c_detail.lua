...
function enableDetail()
	...
	if not bAllValid then
		outputChatBox( "dr_shader_detail: Could not create some things.", 255, 0, 0 )
		disableDetail()
	else
		outputDebugString( "dr_shader_detail: is using technique " .. tec )
...
function getMakeShader(v)
	...
	if renderTarget.isOn then
		if enableNormal then
			shader,tec = dxCreateShader ( "fx/detail_dr_n.fx", 2, renderTarget.distFade[1] )
		else
			shader,tec = dxCreateShader ( "fx/detail_dr.fx", 2, renderTarget.distFade[1] )
		end
	else
